...
class LaplacianLayer(nn.Module):
	def __init__(self):
		...
		w_nom = torch.FloatTensor([[0, -1, 0], [-1, 4, -1], [0, -1, 0]]).view(1,1,3,3).cuda()
		w_den = torch.FloatTensor([[0, 1, 0], [1, 4, 1], [0, 1, 0]]).view(1,1,3,3).cuda()
...
class JointLoss(nn.Module):
	...
	def DeMonGradientLoss(self, prediction_d, mask, gt_d):
		...
		v_mask = torch.mul(mask[:, :-2,:], mask[:, 2:,:])
		v_grad_pred = (prediction_d[:, :-2,:] - prediction_d[:, 2:,:])/(prediction_d[:, :-2,:] + prediction_d[:, 2:,:] + EPSILON)
		v_grad_gt = (gt_d[:, :-2,:] - gt_d[:, 2:,:])/(gt_d[:, :-2,:] + gt_d[:, 2:,:] + EPSILON)
		v_grad_term = v_mask * torch.abs(v_grad_pred - v_grad_gt)

		h_mask = torch.mul(mask[:, :, :-2], mask[:, :, 2:])
		h_grad_pred = (prediction_d[:, :, :-2] - prediction_d[:, :, 2:])/ (prediction_d[:, :, :-2] + prediction_d[:, :, 2:] + EPSILON)
		h_grad_gt = (gt_d[:, :, :-2] - gt_d[:, :, 2:])/ (gt_d[:, :, :-2] + gt_d[:, :, 2:] + EPSILON)
	...
	def Confidence_Loss(self, pred_confidence, mask, pred_d, gt_d):
		...
		if N > 0.5:
			scale_factor = torch.median(gt_d.data[mask.data>0.1]/(pred_d.data[mask.data>0.1] + EPSILON)).item()
			pred_d_aligned = pred_d * scale_factor 

			error = torch.abs(pred_d_aligned.data - gt_d.data)/(gt_d.data + EPSILON)
	...
	def Normal_Loss(self, pred_d, gt_d, mask, normalized_p_3d, p_3d_gt):
		...
		grad_x_pred = (pred_d[:,1:-1,:-2] - pred_d[:,1:-1,2:])/(p_3d_pred[:,0,1:-1,:-2] - p_3d_pred[:,0,1:-1,2:] + EPSILON)
		# grad_x_pred = grad_x_pred * final_mask
		grad_x_gt = (gt_d[:,1:-1,:-2] - gt_d[:,1:-1,2:])/(p_3d_gt[:,0,1:-1,:-2] - p_3d_gt[:,0,1:-1,2:] + EPSILON)
		# grad_x_gt = grad_x_gt * final_mask

		grad_y_pred = (pred_d[:,:-2,1:-1] - pred_d[:,2:,1:-1])/(p_3d_pred[:,1,:-2,1:-1] - p_3d_pred[:,1,2:,1:-1] + EPSILON)
		# grad_y_pred = grad_y_pred * final_mask
		grad_y_gt = (gt_d[:,:-2,1:-1] - gt_d[:,2:,1:-1])/(p_3d_gt[:,1,:-2,1:-1] - p_3d_gt[:,1,2:,1:-1] + EPSILON)
		# grad_y_gt = grad_y_gt * final_mask

		# torch.nn.CosineSimilarity

		norm_pred = torch.sqrt(grad_x_pred * grad_x_pred + grad_y_pred * grad_y_pred + 1.0) + EPSILON
		norm_gt = torch.sqrt(grad_x_gt * grad_x_gt + grad_y_gt * grad_y_gt + 1.0)  + EPSILON
	...
	def L1_inv_loss(self, pred_d_aligned, mask, depth_gt):
		...
		l1_inv_error = torch.abs(1./(depth_gt + EPSILON) - 1./(EPSILON + pred_d_aligned))
	...
	def compute_si_rmse(self, pred_log_d, targets):
		...
		for i in range(0, gt_mask.size(0)):
			...
			mse_full = self.Data_Loss(pred_log_d[i,:,:], gt_mask[i,:,:], log_d_gt[i,:,:])
			mse_env = self.Data_Loss(pred_log_d[i,:,:], env_gt_mask[i,:,:], log_d_gt[i,:,:])
			mse_intra = self.Data_Loss(pred_log_d[i,:,:], human_gt_mask[i,:,:], log_d_gt[i,:,:])
			...
			per_pixel_error = n_full * torch.pow(log_diff,2) + sum_sq_log_diff - 2*sum_log_diff*log_diff;
			...
			inter_error = n_env * torch.pow(log_diff, 2) + sum_sq_log_env_diff - 2*sum_log_env_diff*log_diff;
		...
		# print('si_rmse_full, si_rmse_human, si_rmse_env', si_rmse_full, si_rmse_human, si_rmse_env)
		# sys.exit()
	...
	def compute_l1_rel_error(self, pred_d, targets):
		...
		for i in range(0, gt_mask.size(0)):
			...
			scale_factor = np.linalg.lstsq(np.expand_dims(pred_d_np[gt_mask_np > 1e-8], axis=-1), gt_d_np[gt_mask_np > 1e-8])
			...
			total_full_rel = np.sum(gt_mask_np * np.abs(gt_d_np - pred_d_aligned_np)/(gt_d_np + EPSILON))
	...
	def compute_rmse_error(self, pred_d, targets):
		...
		for i in range(0, gt_mask.size(0)):
			...
			total_full_rmse = np.sum(gt_mask_np * np.square(gt_d_np - pred_d_aligned_np))
			rmse_full += np.sqrt(total_full_rmse/(np.sum(gt_mask_np) + EPSILON))
	...
	def Data_Human_Loss(self, pred_log_d, gt_mask, human_gt_mask, log_d_gt):
		...
		inter_error = n_full * torch.pow(log_diff,2) + sum_sq_log_diff - 2*sum_log_diff*log_diff;
	...
	def __call__(self, input_images, log_pred_d_0, pred_confidence, targets):
		...
		human_mask = 1.0 -  Variable(targets['env_mask'].cuda(), requires_grad = False)
		...
		for i in range(0, num_samples):
			if self.opt.human_data_term > 0.1:
				data_term += self.w_si_mse/num_samples * self.Data_Loss(log_pred_d_0[i,:,:], 
					mask_0[i,:,:], log_d_gt_0[i,:,:]) 
				data_term += self.w_si_mse/num_samples * 0.5 * self.Data_Human_Loss(log_pred_d_0[i,:,:], 
					mask_0[i,:,:], human_gt_mask[i,:,:], log_d_gt_0[i,:,:]) 
			else:
				data_term += self.w_si_mse/num_samples * 1.5 * self.Data_Loss(log_pred_d_0[i,:,:], 
					mask_0[i,:,:], log_d_gt_0[i,:,:]) 

			# confidence_term += self.w_confidence/num_samples * self.Confidence_Loss(pred_confidence[i,:,:], 
				# mask_0[i,:,:], torch.exp(log_pred_d_0[i,:,:]), d_gt_0[i,:,:])

		grad_term += self.w_grad * self.GradientLoss(log_pred_d_0, mask_0, log_d_gt_0)
		grad_term += self.w_grad * self.GradientLoss(log_pred_d_1, mask_1, log_d_gt_1)
		grad_term += self.w_grad * self.GradientLoss(log_pred_d_2, mask_2, log_d_gt_2)
		grad_term += self.w_grad * self.GradientLoss(log_pred_d_3, mask_3, log_d_gt_3)
		grad_term += self.w_grad * self.GradientLoss(log_pred_d_4, mask_4, log_d_gt_4)

		sm_term += self.w_sm1 * self.compute_image_aware_1st_smoothness_cost(log_pred_d_0, input_0)
		sm_term += self.w_sm1 * 0.5 * self.compute_image_aware_1st_smoothness_cost(log_pred_d_1, input_1)
		sm_term += self.w_sm1 * 0.25 * self.compute_image_aware_1st_smoothness_cost(log_pred_d_2, input_2)
		sm_term += self.w_sm1 * 0.125 * self.compute_image_aware_1st_smoothness_cost(log_pred_d_3, input_3)
		sm_term += self.w_sm1 * 0.0625 * self.compute_image_aware_1st_smoothness_cost(log_pred_d_4, input_4)

		sm_term += self.w_sm2 * self.LaplacianSmoothnessLoss(log_pred_d_0, input_0)
		sm_term += self.w_sm2 * 0.5 * self.LaplacianSmoothnessLoss(log_pred_d_1, input_1)
		sm_term += self.w_sm2 * 0.25 * self.LaplacianSmoothnessLoss(log_pred_d_2, input_2)
		sm_term += self.w_sm2 * 0.125 * self.LaplacianSmoothnessLoss(log_pred_d_3, input_3)
		sm_term += self.w_sm2 * 0.0625 * self.LaplacianSmoothnessLoss(log_pred_d_4, input_4)

  
		print('data_term %f'%data_term.item())
		print('grad_term %f'%grad_term.item())
		print('sm_term %f'%sm_term.item())
		# print('confidence_term %f'%confidence_term.item())

		# print('normal_term %f'%normal_term.item())
